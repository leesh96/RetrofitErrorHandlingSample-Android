...
import com.suho.demo.githubsearchsample.remote.response.safeApiCall
...
object SearchRepository {
    ...
    suspend fun getSearchResult(query: String): Flow<Resource<List<GithubRepo>>> = flow {
        ...
        when (val response = safeApiCall { mClient.searchRepositories(query) }) {
            is ApiResponse.Success -> emit(Resource.success(response.data?.items?.map {
                GithubRepo(
                    id = it.id,
                    name = it.name,
            ...
            is ApiResponse.Error -> emit(Resource.error(response.message, null))
        }
    }
}